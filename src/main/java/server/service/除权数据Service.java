...
public class 除权数据Service  extends 爸爸数据Service{
	...
	public String 下载除权导入数据() {
		...
		String[] s除权原始文件全路径 = new String[1];
		s除权原始文件全路径[0]="/Users/haoyan/Desktop/FeihuTool_Convert/除权数据.data";

		String s除权导出文件全路径 = null;
		s除权导出文件全路径 = new StockData除权ConvertTool().输出到除权文件(s除权原始文件全路径);

		if(StringUtils.isEmpty(s除权导出文件全路径)) {
			return s除权导出文件全路径;
		}

		// 存储除权数据（解析原始文件、存入百度网盘）
		String s本地上传文件全路径 = s除权原始文件全路径[0];
		String s网盘存储文件全路径 = "/apps/除权数据" + 取得系统时间_文件名() +".data";
		存储到百度网盘(s本地上传文件全路径, s本地上传文件全路径);
